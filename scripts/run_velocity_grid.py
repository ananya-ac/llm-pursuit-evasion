...
def save_heatmap(matrix, blue_velocities, red_velocities, n_games, output_png):
    ...
    ax.set_yticks(np.arange(len(blue_velocities)))
    ax.set_xticklabels(red_velocities)
    ax.set_yticklabels(blue_velocities)
    ax.set_xlabel("Red Max Velocity", fontsize=AXIS_LABEL_FONTSIZE)
    ax.set_ylabel("Blue Max Velocity", fontsize=AXIS_LABEL_FONTSIZE)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated blue max velocities. Default: 1,2,3,4,5",
    )
    parser.add_argument(
        "--red-velocities",
        type=str,
    ...
    parser.add_argument(
        "--take-gifs",
        action="store_true",
        help="Save one representative GIF for each blue/red velocity pair.",
    )
    parser.add_argument(
        "--gif-dir",
        type=str,
            ...
            for game_idx in range(args.n_games):
                ...
                if need_simulation:
                    ...
                    if need_representative_gif:
                        gif_name = f"blue_v{p_vel}_red_v{e_vel}.gif"
                    ...
                    if (
                        ...
                        if len(simulation.history_red) > 1:
                            incomplete_gif_name = (
                                f"incomplete_blue_v{p_vel}_red_v{e_vel}_game{game_idx}.gif"
                            )
                    ...
                    if (
                        ...
                        if len(simulation.history_red) > 1:
                            failure_gif_name = (
                                f"solver_failure_blue_v{p_vel}_red_v{e_vel}_game{game_idx}.gif"
                            )
