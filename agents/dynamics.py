...
class Agent:
    ...
    def __init__(
        ...
        if agent_type not in (self.BLUE, self.RED):
        ...
        self.omega_max = float(omega_max) if self.include_heading else None

        # Bearing (theta), when present, is a genuinely independent integrator
        # driven by its own control (omega, yaw rate) -- theta_{k+1} = theta_k
        # + dt*omega_k. It never appears in, or is driven by, the position/
        # velocity block below, which is copied unchanged either way: adding
        # heading never alters the translational dynamics.
        self.nx_single = 5 if self.include_heading else 4
        self.nu_single = 3 if self.include_heading else 2
        ...
        if self.include_heading:
            ad_single = np.zeros((5, 5))
            ad_single[0:4, 0:4] = ad_pos
            ad_single[4, 4] = 1.0
            bd_single = np.zeros((5, 3))
            bd_single[0:4, 0:2] = bd_pos
            bd_single[4, 2] = dt
        else:
            ad_single = ad_pos
            bd_single = bd_pos
    ...
    def constant_velocity_traj(self, state, horizon):
        # Only ever used to extrapolate position/velocity (e.g. for intercept
        # prediction); reads just the first 4 components regardless of
        # whether heading is present.
        state = np.asarray(state, dtype=float).reshape(self.nx_single)
        px, py, vx, vy = state[0], state[1], state[2], state[3]
...
class QuadcopterAgent:
    ...
    def __init__(
        ...
        if agent_type not in (self.BLUE, self.RED, self.GENERIC):
