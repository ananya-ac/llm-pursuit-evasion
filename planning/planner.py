...
class PlannerObservation:
    """Everything a BlueRolePlanner (rule-based or LLM-backed) needs to assign roles.

    Bundled as a dataclass so BlueRolePlanner.plan()'s signature stays stable when a
    future LLM planner needs more context to build its prompt.

    The contact_* fields are only populated when a vision planner
    (Experiment II) is in use; text-only planners (Experiment I) ignore them
    and Simulation leaves them at their None defaults otherwise.
    ...
    detected_contact_ids: the Experiment II analogue of detected_red_indices,
    in contact_id (roster) space rather than raw red index -- Simulation
    already filters contact_states/contact_image_paths down to only detected
    contacts before populating them, so this field is technically derivable
    as contact_states.keys(), but is included explicitly so vision planners
    can validate a chosen target_contact_id the same way LLMRolePlanner
    validates target_red_id against detected_red_indices.
    """
    ...
    detected_contact_ids: Optional[List[int]] = None
...
class BlueRolePlanner(ABC):
    """Interface for anything that assigns each blue a BlueRole each planning cycle."""

    @abstractmethod
    def plan(self, obs: PlannerObservation) -> Dict[int, BlueRole]:
        ...
...
class RuleBasedBlueRolePlanner(BlueRolePlanner):
    ...
    Stands in for a future LLM-backed planner behind the same BlueRolePlanner
    interface.
    """
    ...
    def plan(self, obs: PlannerObservation) -> Dict[int, BlueRole]:
        ...
        if not detected_reds:
            ...
            return {
                i: (BlueRole.RECON if i in recon_targets else BlueRole.DEFEND) for i in active_blues
            }
...
class RuleBasedRedRolePlanner(RedRolePlanner):
    """Stub planner: EVADE is chosen per-red, only once a detected blue is
    within min_evade_distance of that specific red (a detected-but-distant
    blue is not yet a reason to break off the attack); otherwise -- no blue
    detected at all, or every detected blue is still beyond the threshold --
    the red ATTACKs the defended region. RECON is not offered by this
    planner. Stands in for a future LLM-backed planner behind the same
    RedRolePlanner interface.
    """

    def __init__(self, min_evade_distance: float = 15.0):
        self.min_evade_distance = float(min_evade_distance)
    ...
    def plan(self, obs: PlannerObservation) -> Dict[int, RedRole]:
        ...
        roles: Dict[int, RedRole] = {}
        for j in range(obs.num_reds):
            p_red = red_states[j * red_nx : j * red_nx + 2]
            nearest_dist = min(
                np.linalg.norm(p_red - x_current[i * blue_nx : i * blue_nx + 2])
                for i in detected_blues
            )
            roles[j] = RedRole.EVADE if nearest_dist <= self.min_evade_distance else RedRole.ATTACK
        return roles
...
def _format_coverage_lines(obs: PlannerObservation) -> List[str]:
    """Shared coverage-region rendering used by both _format_observation
    (Experiment I) and _format_vision_text (Experiment II) -- identical
    staleness-grid text either way, since RECON/coverage tracking is
    entity-agnostic (it tracks arena cells, not what's being detected)."""
    if not obs.coverage_regions:
        return []
    lines = [
...
def _format_observation(obs: PlannerObservation, perspective: str) -> str:
    ...
    lines.extend(_format_coverage_lines(obs))
...
def _call_structured_assignment(
    ...
    include_contact_target: bool = False,
    include_region_target: bool = False,
    max_retries: int = 2,
    user_content_builder: Optional[Callable[[PlannerObservation], list]] = None,
):
    """Shared request/parse plumbing for every LLM role planner (text and
    vision). Retries up to max_retries times on a schema-validation failure --
    not every model routed through OpenRouter reliably honors the strict
    json_schema response_format below, and a retry often just gets a
    compliant response on the next roll. If every attempt fails, the last
    exception propagates; callers (LLMRolePlanner/LLMRedRolePlanner/
    LLMVisionRolePlanner.plan()) catch it and fall back to a safe default
    role for the cycle rather than crashing the whole run.
    ...
    fallback. include_contact_target=True (blue-vision only) is the same
    idea in contact_id space -- adds target_contact_id instead, for
    Experiment II's shuffled red+bird+civilian roster. Mutually exclusive
    with include_target. include_region_target=True (blue only) similarly
    adds a required target_region_id field, letting the planner pick which
    ...
    fallback. Red's schema is unaffected by any of these flags.

    user_content_builder, when given, replaces the default plain-string user
    message (_format_observation(obs, perspective)) with
    user_content_builder(obs) -- a multi-modal OpenAI content list (text +
    image_url parts), for the vision planner's image-attached observation.
    Every other caller omits it and gets identical behavior to before."""
    if include_target and include_contact_target:
        raise ValueError("include_target and include_contact_target are mutually exclusive.")
    properties = {
    ...
    if include_contact_target:
        properties["target_contact_id"] = {
            "type": "integer",
            "description": (
        ...
        response = _create_completion_with_backoff(
            ...
            messages=[
                {"role": "system", "content": system_prompt},
                {
                    "role": "user",
                    "content": (
                        user_content_builder(obs)
                        if user_content_builder is not None
                        else _format_observation(obs, perspective)
                    ),
                },
            ],
            response_format={
...
Respond only with a valid JSON object matching the required schema."""
    ...
    def plan(self, obs: PlannerObservation) -> Dict[int, BlueRole]:
        try:
            ...
            return {i: BlueRole.DEFEND for i in active_blues}
        ...
        for item in parsed.assignments:
            ...
            if role == BlueRole.NEUTRALIZE:
                if item.target_red_id not in detected_reds:
                    ...
                    # agent this cycle, matching RuleBasedBlueRolePlanner's
                    ...
                    roles[item.agent_id] = BlueRole.DEFEND
...
#
# Blue-only: reds, decoy birds, and decoy civilian drones are merged into
# an unlabeled "contacts" list (position/velocity + an attached image, no
# ground-truth tag), so the model must use the image -- not the state
# channel -- to judge whether a contact is a real threat. Adds
# target_contact_id so role assignment also controls which contact gets
# engaged, rather than solve_decentralized's blind nearest-red heuristic.
# Otherwise identical to Experiment I's LLMRolePlanner (RECON, coverage,
# retry/backoff/fallback via the shared _call_structured_assignment) --
# contacts are detection-gated by Simulation before they ever reach this
# module. Red's planner is unchanged from Experiment I.
...
def _format_vision_text(obs: PlannerObservation) -> str:
    ...
    lines.append(
        "contacts (detected only -- bounded by your team's combined field of view; "
        "undetected contacts are NOT listed here but may still exist. Every contact "
        "is UNIDENTIFIED: it may be a real antagonistic drone, a harmless bird, or a "
        "harmless civilian drone -- position and velocity alone may not tell them "
        "apart, use each contact's attached image below to judge):"
    )
...
class LLMVisionRolePlanner(BlueRolePlanner):
    """Vision-augmented blue role planner (Experiment II) -- identical to
    LLMRolePlanner (same roles, same RECON/target_region_id mechanic, same
    retry/backoff/fallback-to-DEFEND robustness via the shared
    _call_structured_assignment) except contacts (reds + decoy birds +
    decoy civilian drones, shuffled, no ground-truth tag) are shown in
    place of red_agents, each with an attached image, via
    PlannerObservation.contact_states/contact_image_paths -- both already
    ...
    def plan(self, obs: PlannerObservation) -> Dict[int, BlueRole]:
        try:
            parsed = _call_structured_assignment(
                ...
                self._system_prompt(),
                obs,
                "blue_vision_role_assignment",
                _BlueVisionAssignmentResponse,
                self._VALID_ROLES,
                perspective="blue",
                include_contact_target=True,
                include_region_target=True,
                user_content_builder=self._build_user_content,
            )
        except Exception as exc:  # noqa: BLE001 -- a non-compliant model response
            # (or a network/API failure) must not crash the whole run: fall
            # back to DEFEND for every active blue this cycle and keep going.
            ...
            active_blues = [i for i in range(obs.num_blues) if i not in disabled]
            print(
                f"[LLM blue-vision] giving up on this cycle's response ({type(exc).__name__}: "
                f"{exc}) -- falling back to DEFEND for all active agents."
            )
            self.fallback_events.append(
                {
        ...
        detected_contacts = (
            set((obs.contact_states or {}).keys())
            if obs.detected_contact_ids is None
            else set(obs.detected_contact_ids)
        )
        valid_regions = {region["region_id"] for region in (obs.coverage_regions or [])}
        ...
        for item in parsed.assignments:
            ...
            if role == BlueRole.NEUTRALIZE:
                if item.target_contact_id not in detected_contacts:
                    ...
                    # targets a contact that has since dropped out of FOV.
                    print(
                        f"[LLM blue-vision] agent {item.agent_id} was assigned {role.value} "
                        f"but target_contact_id={item.target_contact_id} is not a currently "
                        "detected contact -- falling back to DEFEND for this agent this cycle."
                    )
                    self.fallback_events.append(
                        {
                            "sim_step": obs.sim_step,
                            "agent_id": item.agent_id,
                            "kind": "invalid_target_contact_id",
                            "attempted_role": role.value,
                            "invalid_target_contact_id": item.target_contact_id,
                        }
                    )
                    roles[item.agent_id] = BlueRole.DEFEND
                    reasoning[item.agent_id] = (
                        f"(fallback to DEFEND: invalid target_contact_id={item.target_contact_id}) "
                        f"{item.reasoning}"
                    )
                    continue
                ...
                if item.target_region_id not in valid_regions:
                    print(
                        f"[LLM blue-vision] agent {item.agent_id} was assigned RECON but "
                        f"target_region_id={item.target_region_id} is not a valid "
                        "coverage region -- falling back to DEFEND for this agent this cycle."
                    )
                    self.fallback_events.append(
            ...
            print(
                ...
                f"(target_contact={target_str}, target_region={region_str}): {item.reasoning}"
            )
...
class RuleBasedVisionRolePlanner(BlueRolePlanner):
    ...
    tested without any API calls. recon_fraction/min_recon mirror
    RuleBasedBlueRolePlanner's (default off) so RECON+vision can also be smoke
    ...
    def _select_recon_assignments(self, candidates, obs):
        """Identical staleness-ranking logic to
        RuleBasedBlueRolePlanner._select_recon_assignments."""
        if not candidates or not obs.coverage_regions:
            return {}
        n_recon = max(self.min_recon, int(round(len(candidates) * self.recon_fraction)))
        n_recon = min(n_recon, len(candidates), len(obs.coverage_regions))
    ...
    def plan(self, obs: PlannerObservation) -> Dict[int, BlueRole]:
        ...
        for rank, idx in enumerate(order):
            if rank < n_intercept:
                ...
                roles[idx] = BlueRole.DEFEND
                defend_candidates.append(idx)
            if roles[idx] != BlueRole.DEFEND and contacts[idx] is not None:
