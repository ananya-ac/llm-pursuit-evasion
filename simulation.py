...
from solver import (
    DecentralizedPursuitEvasionSolver,
    PerimeterDefenseMinimaxSolver,
    PursuitEvasionMinimaxSolver,
)
from planner import (
    LLMRedRolePlanner,
    LLMRolePlanner,
...
_RED_DRONE_IMAGES = sorted(glob.glob(os.path.join(_ASSETS_DIR, "drones", "red_*.png")))
_BLUE_DRONE_IMAGES = sorted(glob.glob(os.path.join(_ASSETS_DIR, "drones", "blue_*.png")))
_BIRD_IMAGES = sorted(glob.glob(os.path.join(_ASSETS_DIR, "birds", "bird_*.png")))
...
class Simulation:
    def __init__(
        ...
        self.history_birds = []
        self.contact_roster = []  # list of ("red"|"bird", index), position = contact_id
        self.contact_images = {}  # contact_id -> image path
        self.own_images = {}  # blue_id -> image path
    ...
    def _spawn_birds(self):
        ...
        draw per slot). Birds are tactically inert -- pure random-walk motion,
        never captured/escaped/counted toward any win condition -- and exist
        only so a vision-augmented planner has real decoys to misidentify."""
        bird_states = []
        for _ in range(self.num_bird_slots):
            ...
            speed = np.random.uniform(0.3, 1.0) * self.bird_v_max
            vel = speed * np.array([np.cos(angle), np.sin(angle)])
            bird_states.append([pos[0], pos[1], vel[0], vel[1]])
    ...
    def _step_birds(self):
        """Bounded random-walk update for each bird: small random acceleration,
        velocity/position integration, and reflection off the arena boundary."""
        for b in range(self.num_birds):
            idx = b * 4
            px, py, vx, vy = self.bird_state[idx : idx + 4]
            ax, ay = np.clip(
                np.random.normal(0.0, 0.5 * self.bird_a_max, size=2),
                -self.bird_a_max,
                self.bird_a_max,
            )
            vx = float(np.clip(vx + ax * self.dt, -self.bird_v_max, self.bird_v_max))
            vy = float(np.clip(vy + ay * self.dt, -self.bird_v_max, self.bird_v_max))
            px, py = px + vx * self.dt, py + vy * self.dt
            if px < self.arena_min or px > self.arena_max:
                vx = -vx
                px = np.clip(px, self.arena_min, self.arena_max)
            if py < self.arena_min or py > self.arena_max:
                vy = -vy
                py = np.clip(py, self.arena_min, self.arena_max)
            self.bird_state[idx : idx + 4] = [px, py, vx, vy]

    def _build_contact_roster(self):
        """Builds the fixed, per-episode (kind, index) -> contact_id mapping
        used by vision planners: reds and birds shuffled together so a
        contact's id carries no positional hint about its true identity, plus
        a stable image assignment per contact and per own-team blue agent.
        ...
        roster = [("red", j) for j in range(self.num_reds)] + [
        ...
        for contact_id, (kind, _idx) in enumerate(roster):
            pool = _BIRD_IMAGES if kind == "bird" else _RED_DRONE_IMAGES
            if pool:
                self.contact_images[contact_id] = local_rng.choice(pool)

        self.own_images = {}
        if _BLUE_DRONE_IMAGES:
            for i in range(self.num_blues):
                self.own_images[i] = local_rng.choice(_BLUE_DRONE_IMAGES)
    ...
    def _active_contact_states(self):
        ...
        have already been resolved (captured/escaped) -- birds are always
        shown, since they never resolve."""
        states = {}
    ...
    def _resolve_target_override(self, target_contact_ids):
        ...
        counts how many resolve to a bird (a wasted NEUTRALIZE/CAPTURE
        commitment -- birds are never in red_outcomes, so engaging one can
        never earn capture/neutralize credit)."""
        target_override = {}
    ...
    def _compute_detected_indices(self, sim_step):
        """Returns (detected_red_indices, detected_blue_indices): which
        opposing agents each team's combined field of view can see this
        ...
        Own-team visibility is unaffected by this -- it's handled entirely
        in planner._format_observation, not here.
        ...
        return detected_red_indices, detected_blue_indices
    ...
    def run(self):
        ...
        for step in range(self.sim_steps):
            ...
            if self.solver_mode == "decentralized":
                ...
                detected_red_indices, detected_blue_indices = self._compute_detected_indices(step)
                if step % self.planning_interval_steps == 0:
                    active_contact_states = self._active_contact_states()
                    obs = PlannerObservation(
                        ...
                        contact_states=active_contact_states,
                        contact_image_paths=self._active_contact_images(active_contact_states.keys()),
                        own_image_paths=self.own_images,
                    )
            ...
            if self.solver_supports_cbf and (
                ...
                blue_red_cbf_exempt = (
                    {i for i, r in self.role_assignment.items() if r == Role.NEUTRALIZE}
                    if self.solver_mode == "decentralized"
                    else None
                )
                if self.blue.include_heading:
